...
module testbench;
  ...
  initial
    begin
      ...
      pulp_read32(axi_m, 32'hAE030000 + 8'h00, status);

      // setup a RAB slice for the mailbox
      pulp_write32(axi_m, 32'hAE030000 + 8'h20, 32'hA5120000);
      pulp_write32(axi_m, 32'hAE030000 + 8'h28, 32'hA5122000);
      pulp_write32(axi_m, 32'hAE030000 + 8'h30, 32'h1A120000);
      pulp_write32(axi_m, 32'hAE030000 + 8'h38, 32'h7);

      // mailbox config, interrupt enable
      //pulp_read32(axi_m, 32'hA5120000 + 8'h24, status);
      pulp_write32(axi_m, 32'hA5120000 + 8'h24, 4'h6);

      // setup a RAB slice for the L2
      pulp_write32(axi_m, 32'hAE030000 + 8'h40, 32'hA7000000);
      pulp_write32(axi_m, 32'hAE030000 + 8'h48, 32'hA7010000);
      pulp_write64(axi_m, 32'hAE030000 + 8'h50, 32'h1C000000);
      pulp_write32(axi_m, 32'hAE030000 + 8'h58, 32'h7);

      // setup a RAB slice for the L1
      pulp_write32(axi_m, 32'hAE030000 + 8'h60, 32'hA0000000);
      pulp_write32(axi_m, 32'hAE030000 + 8'h68, 32'hA1000000);
      pulp_write64(axi_m, 32'hAE030000 + 8'h70, 32'h10000000);
      pulp_write32(axi_m, 32'hAE030000 + 8'h78, 32'h7);

      // setup a RAB slice for the L3
      pulp_write32(axi_m, 32'hAE0300A0 + 8'h00, 32'h80000000);
      pulp_write32(axi_m, 32'hAE0300A0 + 8'h08, 32'h88000000);
      pulp_write64(axi_m, 32'hAE0300A0 + 8'h10, 36'h078000000);
      pulp_write32(axi_m, 32'hAE0300A0 + 8'h18, 32'h7);

      // setup a RAB slice
      pulp_write32(axi_m, 32'hAE0300A0 + 8'h20, 32'hA0000000);
      pulp_write32(axi_m, 32'hAE0300A0 + 8'h28, 32'hA0001000);
      pulp_write64(axi_m, 32'hAE0300A0 + 8'h30, 36'h078000000);
      pulp_write32(axi_m, 32'hAE0300A0 + 8'h38, 32'h7);

      // addr_virt[32:12] = 0xA0001, addr_virt[16:12] = 0x1
      //pulp_write32(axi_m, 32'hAE030000 + 16'h8080, 32'hA00017); // Set 1, Entry 0
      //pulp_write32(axi_m, 32'hAE030000 + 16'h9080, 32'h078001);

      //pulp_write32(axi_m, 32'hAE030000 + 16'h8084, 32'hA00017); // Set 1, Entry 1
      //pulp_write32(axi_m, 32'hAE030000 + 16'h9084, 32'h078001);

      //pulp_write32(axi_m, 32'hAE030000 + 16'h8088, 32'hA00017); // Set 1, Entry 2
      //pulp_write32(axi_m, 32'hAE030000 + 16'h9088, 32'h078001);

      pulp_write32(axi_m, 32'hAE030000 + 16'h808C, 32'hA00017); // Set 1, Entry 3
      pulp_write32(axi_m, 32'hAE030000 + 16'h908C, 32'h078001);

      // addr_virt[32:12] = 0xA0002, addr_virt[16:12] = 0x2 -> just prepare it, still disable the mapping
      pulp_write32(axi_m, 32'hAE030000 + 16'h8100, 32'hA00026); // Set 2, Entry 0
      pulp_write32(axi_m, 32'hAE030000 + 16'h9100, 32'h078002);

      // start program execution
      pulp_write32(axi_m, 32'hAE000000 + 4'h8, 32'hC0000001);

      // write virtual address to mailbox
      pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'hA0000010); // hit L1
      pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'hA0001020); // hit L2

      pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'hA0000018); // prefetch hit L1
      pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'hA0001028); // prefetch hit L2

      pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'hA0001030); // miss L1 / hit L2
      pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'hA0002030); // miss L2

      pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'hA0001038); // prefetch miss L1 / hit L2
      pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'hA0002038); // prefetch miss L2
      ...
      begin
        ...
        status = 0;
        pulp_read32(axi_m, 32'hAE050000, status);

        // handle miss -> activate entry in L2
        pulp_write32(axi_m, 32'hAE030000 + 16'h8100, 32'hA00027);

        // wake up PE 0
        pulp_write32(axi_m, 32'hA0200800 + 12'h600, 32'h1);

        // wait for sync
        status = 1;
        while (status)
          begin
            pulp_read32(axi_m, 32'hA5120010, status);
            status = status & 32'h1; // empty
            tb_wait(100);
          end
        pulp_read32(axi_m, 32'hA5120008, status);
        ...
        pulp_write32(axi_m, 32'hAE030000 + 16'h8100, 32'hA00026);

        // send sync signal
        pulp_write32(axi_m, 32'hA5120000 + 4'h0, 32'h3000); // HOST_DONE
      end
